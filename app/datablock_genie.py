...
class DataBlockGenie:
    ...
    def set_row_counts(self, row_count: int):
    ...
    def create_spark_df(self, spark_session: SparkSession) -> DataFrame:
        rows, columns = self.generate_data()
        ...
        if isinstance(cols, List):
            for col in cols:
                if col in temp_cols.keys():
                    del temp_cols[col]
                    self.logger.info(f'Column "{col}" has been deleted')
                else:
                    self.logger.warning(f'Column "{col}" was not found')
        else:
            if cols in temp_cols.keys():
                del temp_cols[cols]
                self.logger.info(f'Column "{cols}" has been deleted')
            else:
